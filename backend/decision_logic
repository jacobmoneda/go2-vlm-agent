...
def main():
    ...
    print("[Decision] Starting robot decision system.")

    # Get user prompt
    user_prompt = input("Enter robot command: ")

    # Process prompt
    try:
        task = process_input(user_prompt)

    except InvalidPromptError as error:
        print("[Input Error]", error)
        return

    # If hard-coded input processing cannot understand it,
    # this will eventually go to the LLM fallback
    if task["needs_llm"]:
        print("[Decision] Prompt needs LLM fallback.")
        print("[Decision] LLM is not connected yet.")
        return
    ...
    print("[Decision] Camera started.")
    print("[Decision] Press Ctrl+C to stop.\n")

    try:
        # Continuous loop for behaviours like following
        while True:
