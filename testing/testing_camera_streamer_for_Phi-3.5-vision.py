...
# Set False if you run by SSH / PuTTY without display window
SHOW_CAMERA_WINDOW = True

# Image size for AI model
IMAGE_SIZE = (224, 224)

# Save latest Go2 frame as PNG for checking
SAVE_PNG = True
...
class Go2CameraStreamer:
    ...
    def _video_callback(self, msg: Go2FrontVideoData_):
        if msg.video720p and len(msg.video720p) > 0:
            ...
            if frame is not None:
                # Drop old frame to keep low latency
    ...
    def start(self):
        ...
        print(f"[SDK] Subscribed to Go2 camera stream on interface: {self.interface}")
...
def run_smolvlm2(image):
    ...
    try:
        ...
        # Backup for older Transformers versions
        inputs = processor.apply_chat_template(
            messages,
            add_generation_prompt=True,
            tokenize=True,
...
def main():
    ...
    while True:
        try:
            ...
            # Show Go2 camera window on computer screen
            ...
            # Convert Go2 camera frame to PIL image
